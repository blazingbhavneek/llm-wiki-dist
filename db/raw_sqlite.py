...
class RawSqliteDatabase(BaseDatabase):
    ...
    def _create_core_tables(self) -> None:
        self.connection.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_search_items_node ON search_items(node_id);

            CREATE VIRTUAL TABLE IF NOT EXISTS search_items_fts USING fts5(
                item_id UNINDEXED, node_id UNINDEXED, field UNINDEXED, text
            );
            """)
        ...
        with self.transaction() as conn:
            ...
            item_ids = [
                ...
                for r in conn.execute(
                    "SELECT id FROM search_items WHERE node_id=?", (node_id,)
                ).fetchall()
            ]
            conn.execute("DELETE FROM search_items_fts WHERE node_id=?", (node_id,))
            conn.execute("DELETE FROM search_items WHERE node_id=?", (node_id,))
            if self._dim is not None:
                ...
                for item_id in item_ids:
                    conn.execute(
                        "DELETE FROM vec_search_item WHERE node_id=?", (item_id,)
                    )
    ...
    def vector_search(
        ...
        if table == "vec_search_item":
            # PK column stores the search_items.id; resolve to node to filter
            # active and return (item_id, distance).
            rows = self.connection.execute(
                f"""
                WITH matches AS (
                    SELECT node_id AS item_id, distance FROM {table}
                    WHERE embedding MATCH ? ORDER BY distance LIMIT ?
                )
                SELECT m.item_id AS item_id, m.distance AS distance
                FROM matches m
    ...
    def replace_search_items(self, node_id: str, items: list[dict]) -> None:
        """Delete then re-insert every search_items row (+ FTS + stale vectors)
        for a node. Vectors for the new rows are set separately."""
        with self.transaction() as conn:
            ...
            for item in items:
                conn.execute(
                    ...
                    INSERT INTO search_items
                        (id, node_id, field, text, ordinal, start_char, end_char,
                ...
                conn.execute(
                    "INSERT INTO search_items_fts(item_id, node_id, field, text) "
                    "VALUES(?,?,?,?)",
                    (item["id"], node_id, item["field"], item["text"]),
                )
    ...
    def _delete_search_items_conn(self, conn: sqlite3.Connection, node_id: str) -> None:
        item_ids = [
            r["id"]
            for r in conn.execute(
                "SELECT id FROM search_items WHERE node_id=?", (node_id,)
        ...
        if self._dim is not None:
            for item_id in item_ids:
                conn.execute("DELETE FROM vec_search_item WHERE node_id=?", (item_id,))
    ...
    def search_items_fts_query(self, text: str, limit: int = 150) -> list[dict]:
        ...
        rows = self.connection.execute(
            """
            SELECT s.id AS item_id, s.node_id AS node_id, s.field AS field,
                   s.text AS text, s.ordinal AS ordinal,
                   s.start_char AS start_char, s.end_char AS end_char
            FROM search_items_fts f
            JOIN search_items s ON s.id = f.item_id
            JOIN nodes n ON n.id = s.node_id
            WHERE search_items_fts MATCH ? AND n.status = 'active'
            ORDER BY rank LIMIT ?
            """,
            (query, limit),
        ).fetchall()
    ...
    def get_search_items(self, ids: list[str]) -> dict[str, dict]:
        ...
        rows = self.connection.execute(
            f"""
            SELECT id AS item_id, node_id, field, text, ordinal, start_char, end_char
            FROM search_items WHERE id IN ({placeholders})
            """,
            list(ids),
        ).fetchall()
