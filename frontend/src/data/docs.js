...
function defaultSortDocuments(a, b) {
  return String(a.name || '').localeCompare(String(b.name || ''))
}

function defaultSortTopics(a, b) {
  return String(a.cluster || '').localeCompare(String(b.cluster || ''))
}

// -> {
//   documents: [{ name, cluster, type, nodes[] }],
//   topics: [{ cluster, docs[] }],
//   nextOf,
//   prevOf
// }
//
// Sorting is now configurable from App.jsx.
//
// Example:
//
// buildLibrary(nodes, edges, {
//   sortDocuments: yourDocumentSorter,
//   sortTopicDocuments: yourDocumentSorter,
//   sortTopics: yourTopicSorter,
// })
export function buildLibrary(nodes = [], edges = [], options = {}) {
  const {
    sortDocuments = defaultSortDocuments,
    sortTopicDocuments = defaultSortDocuments,
    sortTopics = defaultSortTopics,
  } = options || {}
  ...
  for (const n of nodes) {
    const name =
      ...
      (n.type === 'exogenous' ? AGENT_DOC : 'Untitled')
  ...
  for (const [name, group] of groups) {
    ...
    const cluster =
      mode(group.map((n) => n.cluster).filter(Boolean)) || 'Uncategorized'

    documents.push({
      name,
      cluster,
      type: group.some((n) => n.type === 'exogenous')
        ? 'exogenous'
        : 'endogenous',
      nodes: ordered,
    })
  ...
  // Topic view: every cluster present at the NODE level, not one-per-doc.
  // A document reappears under each topic it touches.
    ...
    .map(([cluster, docsMap]) => ({
      ...
      docs: [...docsMap.entries()]
        .map(([name, ns]) => ({
          name,
          cluster,
          type: ns.some((x) => x.type === 'exogenous')
            ? 'exogenous'
            : 'endogenous',
          nodes: orderChain(ns, nextOf, prevOf),
        }))
        .sort(sortTopicDocuments),
    }))
    .sort(sortTopics)
