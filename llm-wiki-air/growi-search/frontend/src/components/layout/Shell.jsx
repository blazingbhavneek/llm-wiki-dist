...
export function PlaceholderPage({ icon: Icon = Sparkles, title, text }) {
  return (
    <div className="grid h-full place-items-center bg-white px-6">
      <div className="max-w-[460px] rounded-2xl border border-dashed border-neutral-300 bg-white p-8 text-center shadow-sm">
        <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-blue-50 text-blue-700">
          <Icon size={24} />
        </div>
        <h1 className="mt-4 text-[22px] font-semibold tracking-tight text-neutral-950">
          {title}
        </h1>
    ...
    <div className="mb-5 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-[20px] font-semibold tracking-tight text-neutral-950">
            {title}
          </h1>
        ...
        {aside && (
          <div className="shrink-0 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-[12px] font-semibold text-blue-700">
            {aside}
          </div>
        )}
      </div>
...
export function Centered({ children }) {
  return (
    <div className="grid h-full place-items-center bg-white text-[14px] text-neutral-500">
      {children}
    </div>
  )
}
