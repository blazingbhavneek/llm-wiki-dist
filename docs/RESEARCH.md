...
`proto.py` is a deliberately bounded adaptation of the architecture above, run
against one seed region rather than the whole corpus. The research architecture
is a menu of useful ideas, not a requirement to reproduce a costly scientific
discovery workflow for every documentation question.

The current prototype has three stages:

1. retrieve a small connected region using search plus one-hop graph expansion;
2. select grounded fact-pairs, then turn them into questions in a separate call;
3. search alternate wording, then make one batch review call that rejects
   answered, unsupported, contrived, low-impact, or duplicate questions.

The normal path therefore makes exactly three bounded LLM calls. Model thinking
remains enabled for quality, but each call receives one small job, output tokens
are capped, each call has a short timeout, and the whole run has an independent
deadline. It does not invoke the multi-agent `Researcher.ask()` path.

### Implemented ideas

- **GAPMAP:** candidates use a software-oriented typed gap taxonomy instead of
  free-form labels.
- **FirstResearch:** every survivor records grounded premises, the missing
  relationship, practical impact, targeted searches, and a concrete resolution
  test.
- **HypER:** proposal and review are separate calls. The proposing model never
  accepts its own output.
- **Agentic Workflows:** the initial context includes real one-hop graph
  relationships and targeted retrieval under alternate wording.

The most important anti-triviality rule is enforced twice. A candidate must
connect at least two supplied nodes, and each premise must contain a verbatim
quote that is checked deterministically against that node before review. The
batch reviewer then requires a normal operational use, cross-source reasoning,
and concrete implementation or maintenance impact. Silence about an invented
or implausible scenario is explicitly rejected.

### Deliberate omissions

- LimAgents-style specialist fan-out is omitted. On this corpus it multiplied
  calls and near-duplicate candidates without making the small model's local
  comparison task easier.
- The deep adversarial `ask()` pass is omitted. It is an answer-generation
  agent with iterative tool use, so using it once per candidate made runtime
  unbounded in practice.
- Graph-wide centrality and cluster-gap mining are not implemented; the
  prototype still begins from one seed chunk.
- Novelty is corpus-internal only. Failed retrieval means "not established in
  the indexed material," never "unknown in the world."
- Human calibration, semantic deduplication beyond the batch reviewer, and
  evaluation against later documentation remain future work.
